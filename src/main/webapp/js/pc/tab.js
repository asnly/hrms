...
jQuery.extend(jQuery.easing, {
    ...
    Tab.prototype = {
        ...
        playTo: function(page) {
            ...
            prevPage;  
            var index = page % c.target.length;
            var data = c.target[index];
            switchBackground(data); //切换背景图
...
$(function(){
		...
		var slider_width = $("#slider_width").val();
		$('#js_banner, #js_banner_img li').width(slider_width);
	...
	function switchBackground(target){
		$(target).closest(".slider_img")[0].style.backgroundImage="url("+$(target).attr("background-img-data")+")";
		$(target).closest(".slider_img")[0].style.backgroundColor =$(target).attr("background-color-data");
	}