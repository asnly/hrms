		...
		if(data!=""&&data!=null){
			...
			$.each(addressList,function(i,n){
				var addressItem = JSON.stringify(n);
				 a+="<dl id='"+n.address.id+"' class='item' onclick='addressSelected("+n.address.id+")'><dt><strong class='itemConsignee'>"+n.address.consignee+"</strong></dt><dd><p class='tel itemTel'>"+n.address.mobile+"</p><p class='itemRegion'>"+n.wholeName+"</p><p class='itemRegion'>"+n.address.street+"</p><span class='edit-btn J_editAddr' id='edit"+n.address.id+"' onclick='editAddress("+addressItem+")'>编辑</span></dd><dd style='display:none'><input type='radio' class='Checkout' value='"+n.address.id+"'/></dd></dl>";		 
			});
			a+=b;  
